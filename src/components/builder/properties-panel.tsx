      ...
      return (
        <div className="space-y-3">
          ...
          <Field label="Background Gradient (CSS)">
            <Input value={(p.bgGradient as string) || ""} onChange={(e) => onUpdate({ bgGradient: e.target.value })} placeholder="linear-gradient(to right, #FF0099, #B51760)" className={inputCls} />
          </Field>
          <Field label="Padding (px)">
            <Input type="number" value={p.padding as string} onChange={(e) => onUpdate({ padding: e.target.value })} className={inputCls} />
          </Field>
          <Field label="Max Width (px)">
            <Input type="number" value={p.maxWidth as string} onChange={(e) => onUpdate({ maxWidth: e.target.value })} className={inputCls} />
          </Field>
          <Field label="Top Border Line">
            <button
              onClick={() => onUpdate({ borderTop: !(p.borderTop as boolean) })}
              className={`w-full rounded-md px-2 py-1.5 text-xs font-medium transition-colors ${(p.borderTop as boolean) ? "bg-[#FF0099] text-white" : "bg-white/5 text-white/40 border border-white/10"}`}
            >
              {(p.borderTop as boolean) ? "Enabled" : "Disabled"}
            </button>
          </Field>
        </div>
      );
      ...
      {(["left", "center", "right"] as const).map((a) => (
        <button
          ...
          className={`flex-1 rounded-md px-2 py-1 text-[10px] font-medium transition-colors ${
            value === a ? "bg-[#FF0099] text-white" : "bg-white/5 text-white/40 hover:text-white hover:bg-white/10"
          }`}
        >
